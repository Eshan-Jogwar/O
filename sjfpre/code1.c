...
int main() {
  ...
  while (completed < n) {
    ...
    if (cpu_proc && rq_count > 0) {
      ...
      for (int i = 1; i < rq_count; i++) {
        if (ready_queue[i]->current_cpu_burst <
            ready_queue[shortest]->current_cpu_burst) {
          shortest = i;
        }
      }
      if (ready_queue[shortest]->current_cpu_burst <
    ...
    if (!cpu_proc && rq_count > 0) {
      ...
      cpu_proc = ready_queue[shortest];
      for (int i = shortest; i < rq_count - 1; i++)
  ...
  print_metrics("SJF (Preemptive)", procs, n, time - 1);
  return 0;
}
